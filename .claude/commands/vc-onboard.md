...
4. If curl exits 0: tell the user "Updated to the latest version — reloading and resuming." Then use the Read tool to read `[project-root]/.claude/commands/vc-onboard.md`. Proceed to Phase 0 of the updated skill, following the instructions just read. Do not re-run the version check — the update is already complete. Do NOT stop, do NOT ask the user to re-run the skill — continue executing from Phase 0 immediately.
...
Detect stacks in two passes, cheapest first. **Never use the Read tool for stack
detection** — it is not needed and is the main cause of context bloat on large codebases.

1. **Path indicators** — file extensions, directory names, filenames (e.g. `.tsx`,
   `supabase/`, `manifest.json`, `Dockerfile`). These come free from the Phase 1 file list —
   no additional tool call needed.
2. **Content indicators** — code-level strings (e.g. `createClient`, `auth.uid()`,
   `ipcMain`). For these, use the **Grep tool** (never Read), with `output_mode:
   "files_with_matches"` and `head_limit` set to a small number (5–10) — you only need to
   know a stack is present, not every occurrence. Scope each Grep to the source globs from
   Phase 1 (pass the same glob patterns via Grep's `glob` param) so it never touches
   dependency/build directories. Combine an indicator group into one regex alternation per
   stack (e.g. `stripe|Stripe\(|loadStripe|paymentIntents`) instead of one Grep call per
   string.

| Stack | Indicators | Detection |
|---|---|---|
| React / frontend | `.tsx`, `.jsx`, `.css`, `components/`, `pages/`, `hooks/` | path |
| Supabase | `supabase/`, `.rls.` (path); `createClient`, `auth.uid()`, `storage.` (content) | path + grep |
| PostgreSQL / SQL | `.sql`, `migrations/` (path); `CREATE TABLE`, `ALTER TABLE` (content) | path + grep |
| Node / Express API | `routes/`, `controllers/`, `middleware/`, server `.ts`/`.js` files | path |
| Shell / bash scripts | `.sh`, `.bash`, Makefile (path); `#!/bin/` (content) | path + grep |
| ETL / data pipeline | pipeline file/dir names, `ingest`, `transform`, `load` (path); pandas/dbt/Airflow imports (content) | path + grep |
| Native app (iOS/Android) | `.swift`, `.kt`, `.m`, `.cpp`, Xcode/Gradle project files | path |
| AI agent | agent/memory/context file or dir names (path); `tool_call`, `system_prompt` (content) | path + grep |
| MCP server | `mcp` in path (path); `ListTools`, `CallTool` (content) | path + grep |
| Django | `settings.py`, `urls.py`, `views.py`, `models.py`, `manage.py` | path |
| React Native | `.ios.tsx`, `.android.tsx`, `metro.config.js` (path); `react-native`, `Expo` (content) | path + grep |
| Browser extension | `manifest.json` (path); `manifest_version`, `content_scripts` (content, grep just that one file) | path + grep |
| Serverless / edge | `vercel.json`, `netlify.toml`, `wrangler.toml`, `functions/` (path); Lambda handler signature (content) | path + grep |
| Firebase | `firebase.json`, `firestore.rules` (path); `initializeApp`, `getFirestore` (content) | path + grep |
| GraphQL | `.graphql`, `resolvers/` (path); `typeDefs`, `gql` (content) | path + grep |
| Stripe / payments | `stripe` in path (path); `Stripe(`, `loadStripe`, `paymentIntents` (content) | path + grep |
| Webhooks | `webhook` in path (path); `x-hub-signature`, `svix`, `handleWebhook` (content) | path + grep |
| Electron | `electron` in path (path); `BrowserWindow`, `ipcMain`, `ipcRenderer` (content) | path + grep |
| Config / infra | `.tf`, `.yaml`, Dockerfile, Kubernetes manifests | path |
...
Present the full proposed chunk breakdown before creating anything. Format it as plain
markdown text in your response — **not** as a single code block — so it renders with
real line breaks and headings. Use this shape, one block per chunk with a blank line
between chunks:

```
### [chunk-name] — `[branch-slug]`
Files: [count]
...
Notes: [any split you made and why, or omit this line if none]
```

After all chunks, show the total number of chunks to be created.
