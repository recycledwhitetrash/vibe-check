...
**`vc-audit` version matches `2026-06-20.2`**: proceed silently.
...
Before writing this receipt: ask yourself "Can I paste a specific `+` or `-` line from the diff output I just received in my tool output?" If the answer is no, you have not run Step 2 for this surface. Run the `git diff` command now and return to Step 4b with actual output to quote.</mandatory>
...
**Coverage gate:** Before proceeding to Phase 5, run both of these bash commands:
...
Do not proceed to Phase 5 until: `grep -c "- \[ \]"` = 0 AND `grep -c "^  Verdict:"` ≥ SURFACE_COUNT × PASS_NUMBER. A `[x]` mark without a receipt is not a walked surface.
</mandatory>
