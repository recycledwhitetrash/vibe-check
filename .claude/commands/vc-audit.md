...
**Write a pass-start marker to the artifact before doing anything else.** Append the following line to the pass log section:

```
### Pass N — in progress
```

Replace this with the full pass log entry (per Phase 5) when the pass completes. If the session ends before the pass completes, this marker signals that the pass must be restarted on resume.
...
**Surface walk receipts** (one entry per surface in the surface map — no omissions):
- Surface: [name from surface map]
  Verdict: NEW FINDINGS | CLEAN
  Evidence: src/auth.ts:42-67 — checked session token handling; [what was checked]
- Surface: [name]
  Verdict: CLEAN
  Evidence: src/utils/validate.ts:12 — checked input validation logic; no issues found
