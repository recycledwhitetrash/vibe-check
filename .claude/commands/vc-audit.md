...
<mandatory>**Do not narrate surface status before running Step 2.** Writing any text output to the user about a surface — e.g., "Surface 1: spotifyFetch — CLEAN", "no changes since pass N", "unchanged", "same as last pass" — before completing Steps 2–3 for that surface is prohibited. Run the diff or Read command first; report only after you have the output in context. The first thing you do for each `[ ]` surface is Step 1 (sensitive file check), then Step 2 (diff or Read) — not announcing its status.

**Do not narrate after Step 2 either.** Once the receipt (Step 4b) is written, that receipt is the record — do not add extra prose praising or explaining a routine pass/fail result (e.g., "Good — that assertion correctly catches the bug, confirming the fix works as intended," or announcing you're going to "double-check" something the receipt already covers). Move directly to the next surface or step. Only write user-facing text for something the receipt format doesn't already capture: a phase transition, a gate failure, or a question that needs the user's input.</mandatory>
...
<mandatory>Collect the background subagent result dispatched at the start of Phase 4. **First check whether a `<task-notification>` for this task ID already appeared earlier in this conversation** — if so, its `<result>` is the full output; use that text directly and do not call TaskOutput again. Only call TaskOutput if no notification for this task has appeared yet. If it has not completed and no notification has arrived, continue with other Phase 5 work and let the notification arrive on its own — do not poll TaskOutput in a loop or call it against a task you have reason to believe is still running; a poll against a running task can return a raw partial transcript instead of a clean result.

The subagent prompt used was the one below (for reference — do not re-dispatch). Process the returned output (whichever way you obtained it above): for each finding, check whether the same issue is already recorded as a row with Status = Open in the findings table. If a duplicate exists, skip it. Add any non-duplicate findings to the findings table using the same F-NNN numbering sequence.
...
<mandatory>**Column discipline.** The Severity column is exactly `[severity] (N/10)` and nothing else — never append `[adversarial]`, `[test-integrity]`, or `[INVESTIGATE]` there. Those tags open the Description column, as the first word(s) before the description text. Wrong: `| F-007 | pass 1 | high (7/10) [adversarial][INVESTIGATE] | file:line | text | Open |`. Right: `| F-007 | pass 1 | high (7/10) | file:line | [adversarial][INVESTIGATE] text | Open |`.</mandatory>
...
<mandatory>Collect the background test integrity agent result dispatched at the start of Phase 4. **First check whether a `<task-notification>` for this task ID already appeared earlier in this conversation** — if so, its `<result>` is the full output; use that text directly and do not call TaskOutput again. Only call TaskOutput if no notification for this task has appeared yet. If it has not completed and no notification has arrived, continue with other Phase 5 work and let the notification arrive on its own — do not poll TaskOutput in a loop or call it against a task you have reason to believe is still running; a poll against a running task can return a raw partial transcript instead of a clean result.
...
<mandatory>**Column discipline.** The Severity column is exactly `[severity] (N/10)` and nothing else — never append `[test-integrity]` or `[INVESTIGATE]` there. Those tags open the Description column, as the first word(s) before the description text. Wrong: `| F-008 | pass 1 | medium (6/10) [test-integrity][INVESTIGATE] | file:line | text | Open |`. Right: `| F-008 | pass 1 | medium (6/10) | file:line | [test-integrity][INVESTIGATE] text | Open |`.</mandatory>
